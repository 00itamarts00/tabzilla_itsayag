...
from utils import get_activation, get_optimizer, as_tensor, as_float, as_numpy, as_cpu, SimpleDataset, FastTensorDataLoader, probe_infnan
...
class ConcreteLayer(nn.Module):
    ...
    def forward(self, x, logits, epoch=None):
        ...
        if self.training:
            ...
            weights = F.one_hot(torch.argmax(self.logits, dim=1), self.input_dim).float()
            x = torch.diagonal(x @ weights.T).reshape(-1,1)
        return x, weights
...
class LLPmodel(MLPLayer,ModelIOKeysMixin):
    def __init__(self,device,input_dim, hidden_dims, k,  start_temp,min_temp, batch_norm=None, dropout=None, activation='relu',nr_epochs=1000):
        ...
        self.thetas = torch.nn.Parameter(init.uniform_(torch.zeros(input_dim, self.k)), requires_grad=True) # Q: what is the best way to initialize?
        self.ConcreteLayer = ConcreteLayer(k,1,start_temp,min_temp,nr_epochs)
        
    def forward(self, feed_dict,epoch):
        x = self._get_input(feed_dict)
        # MLP layer to parameterize alpha
        self.logits = super().forward(x).reshape(-1,self.k) # logits shape: n * k Q: should I put the structure as it is or should I instantiate a MLP inside the class instead? 
        selected_predictor, weights = self.ConcreteLayer(x@self.thetas,self.logits, epoch) # X@self.thetas shape: n * k
    ...
    def get_weights(self,X,epoch):
        self.ConcreteLayer.eval()
        with torch.no_grad():
            selected_predictor, weights = self.ConcreteLayer(X@self.thetas,self.get_logits_torch(X), epoch)
...
class LLP(object):
    ...
    def __init__(self,device="cuda",input_dim=100, hidden_dims=[30], k=3, start_temp=10, min_temp=1e-2,optimizer="Adam",learning_rate=1e-5,weight_decay=1e-3,batch_size=100,batch_norm=None, dropout=None, activation='relu',nr_epochs=1000):
        ...
        self._model = LLPmodel(device,input_dim, hidden_dims, k, start_temp,min_temp, batch_norm, dropout, activation,nr_epochs)
    ...
    def fit(self, X, y, valid_X=None, valid_y=None, 
        ...
        self.train(data_loader, val_data_loader, verbose, print_interval,meters)
    ...
    def get_weights(self,X,epoch):
        self._model.eval()
        with torch.no_grad():
            selected_predictor, weights = self._model.get_weights(torch.from_numpy(X).float().to(self.device),epoch)
        return selected_predictor, weights
